 ...
 */
function embark_form_system_theme_settings_alter(&$form, FormStateInterface $form_state) {
  ...
  $form['custom_css']['embark_custom_css_on'] = array(
    '#type' => 'checkbox',
    '#title' => t('Enable custom CSS'),
    '#default_value' => theme_get_setting('embark_custom_css_on'),
  );
  if (file_exists('public://embark-custom.css'))
    $custom_file = file_get_contents('public://embark-custom.css');
  else
    $custom_file = '';

  $form['custom_css']['embark_custom_css'] = array(
    '#type' => 'textarea',
    '#title' => t('Custom CSS'),
    '#rows' => 14,
    '#resizable' => TRUE,
    ...
    '#states' => [
      'invisible',
      'visible' => [
        'input[name="embark_custom_css_on"]' => ['checked' => TRUE],
      ],
    ],
  );
 ...
 */
function embark_form_system_theme_settings_submit(&$form, FormStateInterface $form_state) {
  file_unmanaged_save_data($form_state->getValue('embark_custom_css'), 'public://embark-custom.css', FILE_EXISTS_REPLACE);
}
